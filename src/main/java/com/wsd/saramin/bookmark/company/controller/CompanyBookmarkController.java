...
public class CompanyBookmarkController {
    ...
    @Operation(summary = "사용자의 회사 북마크 조회", description = "특정 사용자의 모든 회사 북마크를 페이지네이션과 정렬 기준으로 조회합니다.")
    @GetMapping
    public ResponseEntity<List<CompanyBookmarkDTO>> getBookmarks(
            @Parameter(description = "사용자 ID") @RequestParam Long userId,
            @Parameter(description = "페이지 번호") @RequestParam(defaultValue = "1") int page) {
        List<CompanyBookmarkDTO> bookmarks = companyBookmarkService.getBookmarks(userId, page);
